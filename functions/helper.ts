...
export function formatHumanReadable(scores) {
  return scores
    .map(
      (s) =>
...
export function calculateAvoidAny(sourceFile) {
  const anyCount = sourceFile.getDescendantsOfKind(
    SyntaxKind.AnyKeyword,
  ).length;
  const typeRefCount =
    sourceFile.getDescendantsOfKind(SyntaxKind.TypeReference).length || 1;
  return 1 - anyCount / typeRefCount;
}

export function calculateReturnTypes(sourceFile) {
...
export function calculateAccessModifiers(sourceFile) {
  ...
  classes.forEach((cls) => {
    cls.getMembers().forEach((member) => {
      members++;
      if (member.getModifiers().length > 0) withModifiers++;
    });
  });
  return members === 0 ? 1 : withModifiers / members;
}

export function calculateComplexity(sourceFile) {
  const functions = sourceFile.getFunctions();
  if (functions.length === 0) return 1;
  ...
  functions.forEach((f) => {
    ...
    complexity += f
      .getDescendantsOfKind(SyntaxKind.BinaryExpression)
      .filter(
        (expr) => expr.getOperatorToken().getKind() === SyntaxKind.BarBarToken,
      ).length;
...
export function calculateNesting(sourceFile) {
  const functions = sourceFile.getFunctions();
  if (functions.length === 0) return 1;
  let maxNesting = 0;
...
export function calculatePreferConst(sourceFile) {
...
export function calculateAvoidAssertions(sourceFile) {
  const assertions =
...
export function calculateUseReadonly(sourceFile) {
  const properties = sourceFile.getDescendantsOfKind(
    SyntaxKind.PropertyDeclaration,
  );
  if (properties.length === 0) return 1;
  const readonlyCount = properties.filter(
    (p) => p.getStructure().isReadonly,
  ).length;
  return readonlyCount / properties.length;
}

export function calculateAvoidVar(sourceFile) {
  return sourceFile
    .getDescendantsOfKind(SyntaxKind.VariableDeclaration)
    .some(
      (v) =>
...
export function calculateOptionalChaining(sourceFile) {
  // Improved: Check for patterns like x && x.y (manual) vs x?.y (modern)
  let manualNullChecks = 0;
  let modernOperators = 0;
  sourceFile.getDescendantsOfKind(SyntaxKind.BinaryExpression).forEach((e) => {
    if (e.getOperatorToken().getKind() === SyntaxKind.AmpersandAmpersandToken) {
      manualNullChecks++;
    }
  });
...
export function calculateScore(metrics: Record<string, number>) {
  const weightedSum = Object.entries(metrics).reduce(
    (sum, [key, value]) => sum + weights[key] * value,
    0,
  );
  return (
    (weightedSum / Object.values(weights).reduce((sum, w) => sum + w, 0)) * 100
  );
}
